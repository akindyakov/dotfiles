...
def guess_date_from_str(s):
    ...
    if r is not None:
        y = r.group()
        try:
            dt = datetime.datetime(year=int(y) + 2000, month=1, day=1)
            return dt
        except ValueError:
            pass
    return None
...
def guess_create_time(file_path):
    ...
    if not dates:
        d = guess_date_from_str(os.path.basename(file_path))
        if d is not None:
            dates.append(d)
        else:
            ...
            if d is not None:
                dates.append(d)
    if not dates:
        logging.warning("Creation date is absent in exif attributes, use filesystem modification time")
        dates.append(creation_date(file_path))
    return min(dates)
...
def pick_up_filename(dir_path, basename):
...
def process_file(file_path, dst_dir):
    ...
    new_file_name = pick_up_filename(dst_dir, basename)
